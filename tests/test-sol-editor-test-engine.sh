...
grep -F -- '--scope engine' "$wrapper" >/dev/null
grep -F 'args=(-file "${engine_files[@]}")' "$wrapper" >/dev/null

printf 'editor sol.pk3 resource and test-engine override tests passed\n'
