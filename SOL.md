...
`v0.4.0` consumes SOL Engine v0.4 bundle contract 2, wadpack contract 3, and
SOLDEFAULTS contract 1 while preserving story contract 1. Story authoring and
production level design remain parked.

The current E1M1 is deliberately non-production regression content. v0.4 expands
it from the narrow v0.1 graybox into a deterministic systems testbed for engine,
editor, lighting, ambience, renderer, resource, weapon, monster, and object
coverage.

## Engine-owned resource authority

The authoritative v0.4 resource files now live in the sibling engine checkout:
...
`tools/sol-generate-e1m1.py` is the deterministic source of the test map. The
v0.4 fixture contains ten functional room themes represented by 36 connected
sectors across a two-dimensional layout rather than a linear strip.
...
`tools/sol-validate-e1m1.py` rejects disconnected geometry, reduced material or
lighting coverage, missing weapons/starts, inadequate prop coverage, invalid
bounds, and contract drift.
...
- `tools/sol-generate-e1m1.py`: deterministic E1M1 systems-test generator.
- `tools/sol-validate-e1m1.py`: E1M1 systems-test validator.
...
- `sol-project/maps/e1m1/`: tracked deterministic testbed metadata/layout.
